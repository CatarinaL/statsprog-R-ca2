...
convert_timezone <- function(x){
  #print(x)
  hour <- as.numeric(get_hour(x["time"]))
  #print(x["timezone"])
  ...
  #print(hour)
  if(hour >= 24){ #if the hour is 24 or more, then it restarts from 0 and date is +1
    hour <- (hour %% 24)
    x["date"] <- as.Date(x["date"]) + 1
  }
...
new_times <- apply(new_clicks, 1, convert_timezone)
new_clicks$new_times = new_times
